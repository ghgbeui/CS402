...
def read_trace_file(filename):
    """Reads a trace file and extracts addresses.
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            if len(parts) == 2 and parts[0] == '2':  # Check if it's an instruction fetch
...
def plot_histogram(addresses, title):
    ...
    # Set the range of addresses to plot
    min_address = min(non_zero_addresses)
    max_address = max(non_zero_addresses)
    plot_range = max_address - min_address

    # Plot histogram
    plt.figure(figsize=(10, 6))
    plt.hist(addresses, bins=np.arange(min_address, max_address+1), color='skyblue', edgecolor='black')
    plt.title(title)
    plt.xlabel('Address (Decimal)')
    plt.ylabel('Occurrences')
    plt.xlim(min_address, max_address)  # Set x-axis limits
    plt.grid(True)
    plt.show()
