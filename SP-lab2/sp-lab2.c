...
void searchsalaryinrange(struct Employee employees[], int num_employees, int min, int max)
{
    for(int i = 0; i<num_employees; i++)
    {
        printf("Checking employee ID: %d with salary: %.2f\n", employees[i].id, employees[i].salary); // Debug line
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        case 9:
            int min = 0;
            int max = 0;
            printf("Enter the min salary: ");
            scanf("%d", &min);
            printf("Enter the max salary: ");
            scanf("%d", &max);
            searchsalaryinrange(employees, num_employees, min, max);
            break;
        default:
            printf("Wrong number!!\n");
            break;
